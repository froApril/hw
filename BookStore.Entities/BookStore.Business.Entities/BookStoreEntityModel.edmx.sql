...
-- --------------------------------------------------
-- Date Created: 03/26/2020 02:12:43
-- Generated from EDMX file: C:\Users\ASUS\Desktop\BookStore (1)\BookStore.Entities\BookStore.Business.Entities\BookStoreEntityModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_PurchasedHistoryUser]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[PurchasedHistories] DROP CONSTRAINT [FK_PurchasedHistoryUser];
GO
IF OBJECT_ID(N'[dbo].[FK_PurchasedHistoryMedia]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[PurchasedHistories] DROP CONSTRAINT [FK_PurchasedHistoryMedia];
GO
...
GO
IF OBJECT_ID(N'[dbo].[PurchasedHistories]', 'U') IS NOT NULL
    DROP TABLE [dbo].[PurchasedHistories];
GO
...
-- Creating table 'PurchasedHistories'
CREATE TABLE [dbo].[PurchasedHistories] (
    [Id] int IDENTITY(1,1) NOT NULL,
    [Rating] bit  NULL,
    [RateValue] int  NULL,
...
-- Creating primary key on [Id] in table 'PurchasedHistories'
ALTER TABLE [dbo].[PurchasedHistories]
ADD CONSTRAINT [PK_PurchasedHistories]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [User_Id] in table 'PurchasedHistories'
ALTER TABLE [dbo].[PurchasedHistories]
ADD CONSTRAINT [FK_PurchasedHistoryUser]
    FOREIGN KEY ([User_Id])
    REFERENCES [dbo].[Users]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_PurchasedHistoryUser'
CREATE INDEX [IX_FK_PurchasedHistoryUser]
ON [dbo].[PurchasedHistories]
    ([User_Id]);
GO

-- Creating foreign key on [Media_Id] in table 'PurchasedHistories'
ALTER TABLE [dbo].[PurchasedHistories]
ADD CONSTRAINT [FK_PurchasedHistoryMedia]
    FOREIGN KEY ([Media_Id])
    REFERENCES [dbo].[Media]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_PurchasedHistoryMedia'
CREATE INDEX [IX_FK_PurchasedHistoryMedia]
ON [dbo].[PurchasedHistories]
    ([Media_Id]);
GO
